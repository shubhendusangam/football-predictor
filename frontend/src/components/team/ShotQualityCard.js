/**
 * Shot Quality Analytics Card Module
 * ===================================
 *
...
function createCircularProgress(score) {
    ...
    const progressCircle = createSvgElement('circle', {
        ...
        'stroke-dashoffset': `${circumference}` // Start at 0, animate to value
    });
...
function drawSparkline(canvas, trendData) {
    ...
    // Get data arrays
    const shots = trendData.map(d => d.shots || 0);
    const goals = trendData.map(d => d.goals || 0);
...
function createSparklineChart(trendData) {
    ...
    canvas.width = 300; // Will be scaled by CSS
    canvas.height = 60;
...
function normalizeQualityScore(qualityScore) {
    // Backend returns 0-10 scale, convert to 0-100
 ...
 */
export function renderShotQualityCard(container, teamStats, leagueAverages = LEAGUE_AVERAGES) {
    if (!container) {
    ...
    if (!teamStats) {
        const errorCard = createErrorCard('No data available');
    ...
    const teamName = teamStats.teamName || 'Unknown Team';
    const isHome = teamStats.isHome !== undefined ? teamStats.isHome : null;
    const qualityScore = normalizeQualityScore(teamStats.qualityScore);
    const shotAccuracy = teamStats.shotAccuracy || 0;
    const conversionRate = teamStats.conversionRate || 0;
    const shotsTrend = Array.isArray(teamStats.shotsTrend) ? teamStats.shotsTrend : [];
 ...
 */
export function renderShotQualityLoading(container) {
    if (!container) return;
    container.innerHTML = '';
 ...
 * @param {string} message - Error message
 */
export function renderShotQualityError(container, message) {
    if (!container) return;
    container.innerHTML = '';
 ...
 */
export async function fetchAndRenderShotQualityCard(container, teamName, isHome = null) {
    ...
    try {
        ...
        let url = `${baseUrl}/api/teams/${encodeURIComponent(teamName)}/shot-quality`;
        ...
        let statsToRender;
        if (isHome !== null && data.home && data.away) {
...
export { LEAGUE_AVERAGES, NEAR_AVERAGE_THRESHOLD, RATING_LEVELS };
